...
<table class="lp-pmpro-membership-list show-desktop">
	...
	<tr class="lp-pmpro-header">
		...
		foreach ( $levels as $index => $level ):
			...
			<th class="header-item list-item<?php echo $class_count . ' position-' . $index; ?>">
				...
				<div class="lp-price">
                	<span class="amount">
						<?php Manual__IP_Course::instance()->get_addon__membership_level_price( $level ); ?>
					</span>
				</div>
	...
	<tr class="item-row">
		<td class="list-main item-td item-desc"><?php esc_html_e( 'Number of courses', 'manual' ); ?></td>
		<?php
		foreach ( $levels as $index => $level ) {
		...
		<?php foreach ( $levels as $index => $level ):
			...
			<td class="list-item">
				<?php if ( empty( $current_user->membership_level->ID ) || ! $current_level ) { ?>
					<a class="custom-botton" href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ) ?>"><?php _e( 'GET IT NOW', 'manual' ); ?></a>
				<?php } elseif ( $current_level ) { ?>
					<?php
					if ( pmpro_isLevelExpiringSoon( $current_user->membership_level ) && $current_user->membership_level->allow_signups ) {
						?>
						<a class="custom-botton"
						   href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ) ?>"><?php _e( 'Renew', 'manual' ); ?></a>
						<?php
					} else {
						?>
						<a class="custom-botton disabled" href="<?php echo pmpro_url( 'account' ) ?>"><?php _e( 'Your Level', 'manual' ); ?></a>
						<?php
					}
					?>
...
<div class="lp-pmpro-membership-list show-mobile">
	...
	foreach ( $levels as $index => $level ) {
		...
		?>
		<div class="manual-membership-pricing">
			<div class="header-item">
				<div class="lp-price">
					<span class="amount">
						<?php Manual__IP_Course::instance()->manual__get_membership_level_price( $level ); ?>
					</span>
				</div>
			...
			<div class="lp-pmpro-main">
				<ul class="features">
					<li><?php echo esc_html__( 'Number of courses: ', 'manual' ) . ' ' . $count; ?></li>
					<?php
					if ( ! empty( $list_courses ) ) {
						foreach ( $list_courses as $key => $course_item ) {
							if ( ! in_array( $level->id, $course_item['level'] ) ) {
			...
			<div class="lp-pmpro-footer">
				<?php if ( empty( $current_user->membership_level->ID ) || ! $current_level ) { ?>
					<a class="custom-botton pmpro_btn-select"
					   href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ); ?>">
						<?php esc_html_e( 'Get it now', 'manual' ); ?>
					</a>
				<?php } elseif ( $current_level ) { ?>
					<?php
					if ( pmpro_isLevelExpiringSoon( $current_user->membership_level ) && $current_user->membership_level->allow_signups ) { ?>
						<a class="custom-botton pmpro_btn-select"
						   href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ); ?>">
							<?php esc_html_e( 'Renew', 'manual' ); ?>
						</a>
					<?php } else { ?>
						<a class="custom-botton disabled"
						   href="<?php echo pmpro_url( 'account' ); ?>">
							<?php esc_html_e( 'Your level', 'manual' ); ?>
						</a>
					<?php } ?>
				<?php } ?>
			</div>
