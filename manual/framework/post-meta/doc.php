...
<div style="background: #F8F7F7; border: 1px solid #E4E4E4;  padding: 8px 5px 5px 20px; margin:20px 0px;">
  <h3><?php echo esc_html__('Documentation Access Control', 'manual' ); ?></h3>
...
?>
    <br>
    <p class="description"><?php echo esc_html__('Documentation will limit to above define user roles', 'manual' ); ?></p>
  </div>
...
<input type="text" name="doc_cat_icon_name" id="doc_cat_icon_name" size="40" />
...
function manual_doc_category_field_edit( $tag, $taxonomy ) {
	...
	$icon_name = 'doc_cat_icon_name_' . $tag->term_id;
...
<tr class="form-field">
  ...
  <td><input type="text" name="doc_cat_icon_name" id="doc_cat_icon_name" size="40" value="<?php echo esc_html($icon_name_code); ?>" />
...
function manual_doc_category_field_save( $term_id, $tt_id ) {
	...
	$option_cat_icon_name = 'doc_cat_icon_name_' . $term_id;
	...
	if ( isset( $_POST['doc_cat_icon_name'] ) && $_POST['doc_cat_icon_name'] != '' ) {           
        update_option( $option_cat_icon_name, stripslashes($_POST['doc_cat_icon_name']) );
    } else {
