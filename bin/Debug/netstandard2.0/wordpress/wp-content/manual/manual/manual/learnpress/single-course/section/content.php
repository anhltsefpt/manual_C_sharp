            ...
            <li class="<?php echo join( ' ', $item->get_class() ); ?>">
				...
				if ( $item->is_visible() ) {
					...
					if ( $user->can_view_item( $item->get_id() ) ) {
						?>
                        <a class="section-item-link <?php do_action( 'manual__learnpress_before_title', $item, $section, $course ); ?>  " href="<?php echo $item->get_permalink(); ?>"> <?php learn_press_get_template( 'single-course/section/content-item.php', array(
								'item'    => $item,
								'section' => $section
							) ); ?>
                        </a>
...
<?php } else { ?>
	...
	<?php learn_press_display_message( __( 'No items in this section', 'manual' ) ); ?>
