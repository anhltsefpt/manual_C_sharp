 ...
 * @version   2.6.1 for parent theme Manual for publication on ThemeForest
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					...
					// Edit for Manual theme: change the return URL to the Manual Plugin page
						echo '<p><a href="', esc_url( admin_url( 'admin.php?page=manual-admin-plugins' ) ), '" target="_parent">', esc_html( $this->strings['return'] ), '</a></p>';
					} else {
		...
		public function notices() {
			...
			// Edit for Manual theme - Display nag on plugins page too.
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				...
				// Edit for Manual theme - Extra Add (added ! $plugin['required'] to the conditional)
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					// Edit for Manual theme - Extra Add
					$rendered = '<span class="manual-tgmpa-modified-heading" style="margin-top:-0.4em">' . esc_html__( 'Installation/Upgrade Of Required Plugins Needed', 'manual' ) . '</span>';
		...
		protected function create_user_action_links_for_notice( $install_count, $update_count, $activate_count, $line_template ) {
			...
			$action_links = array(
				...
				'update'   => '',
				'activate' => '',
				// Edit for Manual theme: delete the dismiss button to make sure REQUIRED plugin is installed/updated
				//'dismiss'  => $this->dismissable ? '<a href="' . esc_url( wp_nonce_url( add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ), 'tgmpa-dismiss-' . get_current_user_id() ) ) . '" class="dismiss-notice" target="_parent">' . esc_html( $this->strings['dismiss'] ) . '</a>' : '',
				'dismiss' => '',
			);

			// Edit for Manual theme: add class="button-primary"
			$link_template = '<a class="button-primary" href="%2$s">%1$s</a>';
		...
		public function get_tgmpa_status_url( $status ) {
			// Edit for Manual theme: append #manual-theme-install-plugins to the url.
			return add_query_arg(
				array(
					'plugin_status' => urlencode( $status ). '#manual-theme-install-plugins',
				),
				$this->get_tgmpa_url()
			);
		}
